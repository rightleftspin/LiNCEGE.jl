...
function translationally_invariant_clusters(
        ...
        all_cluster_vertices = grow_lower(lattice, start, max_order)

        println(length(all_cluster_vertices))
        println("here also")
        ...
        trans_invar_clusters = unique(
                c -> c[1],
                [
                        (Cluster(lattice, cluster_vertices), cluster_vertices) for cluster_vertices in all_cluster_vertices
                ],
        )
        ...
        #@profile grow_lower(lattice, start, max_order)
        #Profile.clear()
        #@profile grow_lower(lattice, start, max_order)
